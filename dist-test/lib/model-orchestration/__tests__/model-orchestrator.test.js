...
describe('ModelOrchestrator', () => {
    ...
    beforeEach(() => {
        mockRegistry = {
            ...
            getModelChain: jest.fn().mockImplementation(async (requirements) => {
                return (0, test_helpers_1.createMockModelChain)({
        ...
        orchestrator = new model_orchestrator_1.ModelOrchestrator(mockRegistry);
    });
    it('should execute task through model chain', async () => {
        const task = (0, test_helpers_1.createTestTask)('test_task', 'quality', {
    ...
    it('should handle task without review for speed priority', async () => {
        const task = (0, test_helpers_1.createTestTask)('test_task', 'speed', {
            primaryCapability: 'code'
        });
        const result = await orchestrator.handleTask(task);
        expect(result.success).toBe(true);
    ...
    it('should handle task failure', async () => {
        const failingModel = (0, test_helpers_1.createMockModel)({
            id: 'failing-model',
            name: 'Failing Model',
    ...
    it('should retry failed operations', async () => {
        let attempts = 0;
        const retriableModel = (0, test_helpers_1.createMockModel)({
            id: 'retriable-model',
