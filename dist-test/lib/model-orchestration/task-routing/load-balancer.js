...
class LoadBalancer {
     ...
     */
    async distributeLoad(tasks, nodes) {
    ...
    calculateNodeLoad(node) {
        ...
        return ((resources.memory * 0.4) +
            (resources.cpu * 0.4) +
            (resources.tokens * 0.2));
    }
    selectOptimalNode(impact, distribution, nodes) {
        ...
        for (const node of healthyNodes) {
            ...
            if (!this.meetsNodeConstraints(impact, node))
                continue;
            // Then check load constraints
            if (projectedLoad >= this.config.maxNodeUtilization)
                continue;
    ...
    meetsNodeConstraints(impact, node) {
        if (!impact.constraints)
            return true;
        const resources = node.availableResources;
        return ((!impact.constraints.maxMemory || resources.memory >= impact.constraints.maxMemory) &&
            (!impact.constraints.maxCpu || resources.cpu >= impact.constraints.maxCpu));
    }
    assignTaskToNode(impact, nodeId, distribution) {
        const allocation = {
    ...
    canMoveAllocation(allocation, targetNodeId, nodes) {
        ...
        return (resources.memory >= allocation.resources.memory &&
            resources.cpu >= allocation.resources.cpu &&
            resources.tokens >= allocation.resources.tokens);
    }
    moveAllocation(allocation, fromNodeId, toNodeId, distribution) {
    ...
    updateBalanceMetrics(distribution) {
        let totalMemory = 0;
        let totalCpu = 0;
        let totalTokens = 0;
        let nodeCount = 0;
        ...
        if (nodeCount > 0) {
            distribution.balanceMetrics.utilization = {
                memoryUtilization: totalMemory / (nodeCount * 1024),
    ...
    calculateTotalLoad(allocations) {
        return allocations.reduce((total, allocation) => {
            const resourceLoad = ((allocation.resources.memory / 1024) * 0.4 +
    ...
    estimateTaskLoad(task) {
        ...
        return (((constraints.maxMemory || 512) / 1024) * 0.4 +
            (constraints.maxCpu || 0.5) * 0.4 +
            ((constraints.maxLatency || 500) / 1000) * 0.2);
    }
    createInitialDistribution(nodes) {
        return {
    ...
    async calculateLoadAdjustment(metrics) {
        const changes = [];
    ...
    async applyLoadAdjustment(adjustment) {
        ...
        for (const change of adjustment.changes) {
            ...
            if (change.adjustment < 0) {
                ...
                const sortedByLoad = [...allocations].sort((a, b) => this.calculateTotalLoad([b]) - this.calculateTotalLoad([a]));
                let removedLoad = 0;
                const toRemove = [];
                ...
                this.currentDistribution.nodeAllocations.set(change.nodeId, allocations.filter(a => !toRemove.includes(a)));
            }
        }
    }
    ...
    needsOptimization(metrics) {
        return (metrics.loadVariance > this.config.targetLoadVariance ||
    ...
    async performOptimization(metrics) {
        // Identify problematic nodes
        const problemNodes = this.currentDistribution.healthStatus
            .filter(node => node.metrics.responseTime > this.config.healthyResponseTime ||
    ...
    async createSpikeStrategy(incoming) {
        return {
            requiresAction: true,
            recommendations: [
                {
    ...
    needsRebalancing(metrics) {
        ...
        const needsRebalance = (metrics.loadVariance > this.config.targetLoadVariance ||
            metrics.maxUtilization > this.config.maxNodeUtilization ||
            metrics.averageResponseTime > this.config.healthyResponseTime ||
            metrics.errorRate > this.config.maxErrorRate);
    ...
    mapPriorityToAllocation(priority) {
        switch (priority) {
