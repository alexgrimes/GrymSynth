...
describe('CapabilityScorer Performance', () => {
    ...
    it('should handle concurrent operations efficiently', async () => {
        ...
        const start = perf_hooks_1.performance.now();
        await Promise.all(operations.map(op => scorer.recordSuccess(op.modelId, op.capability, {
            latency: 100 + Math.random() * 50,
            resourceUsage: 0.3 + Math.random() * 0.2
        })));
        const duration = perf_hooks_1.performance.now() - start;
    ...
    it('should maintain performance under sustained load', async () => {
        ...
        for (let i = 0; i < numOperations; i++) {
            ...
            timings.push(perf_hooks_1.performance.now() - start);
...
//# sourceMappingURL=capability-scorer.perf.test.js.map