...
describe('Error Performance Tests', () => {
    ...
    describe('Error Processing Performance', () => {
        ...
        it('should maintain performance under error load', () => {
            const errors = Array(100).fill(null).map((_, i) => new Error(`Load test error ${i}`));
            const timings = [];
        ...
        it('should efficiently track error patterns', () => {
            ...
            for (let i = 0; i < iterations; i++) {
                const ErrorClass = globalThis[errorTypes[i % errorTypes.length]];
                healthState = (0, health_1.createErrorHealthState)(new ErrorClass(`Pattern test error ${i}`));
                healthState.metrics.totalOperations++;
            }
        ...
        it('should handle concurrent errors', async () => {
            ...
            const errorPromises = Array(concurrentErrors).fill(null).map(async (_, i) => {
                try {
                    ...
                    return (0, health_1.createErrorHealthState)(error);
                }
            });
            const startTime = performance.now();
...
//# sourceMappingURL=error-performance.test.js.map