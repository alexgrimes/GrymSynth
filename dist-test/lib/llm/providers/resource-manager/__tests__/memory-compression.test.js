...
describe('Memory Compression', () => {
    ...
    describe('Compression Triggers', () => {
        it('should trigger optimization when memory pressure exceeds threshold', async () => {
            ...
            const message = (0, test_helpers_1.createTestMessage)('test message with significant content '.repeat(50));
            await resourceManager.addMessage(TEST_CONTEXT_ID, message);
            expect(optimizeSpy).toHaveBeenCalled();
        });
        it('should respect optimization threshold settings', async () => {
            const customManager = new resource_manager_1.ResourceManager({
                maxMemoryUsage: 1000,
                optimizationThreshold: 0.9,
                cleanupInterval: 1000
            });
            await customManager.initializeContext(TEST_CONTEXT_ID, test_helpers_1.DEFAULT_MODEL_CONSTRAINTS);
            const optimizeSpy = jest.spyOn(customManager, 'optimizeResources');
            const message = (0, test_helpers_1.createTestMessage)('small message');
        ...
        it('should emit resource pressure events under memory pressure', async () => {
            ...
            const message = (0, test_helpers_1.createTestMessage)('large message '.repeat(100));
            await resourceManager.addMessage(TEST_CONTEXT_ID, message);
    ...
    describe('Compression Effectiveness', () => {
        it('should reduce memory usage after optimization', async () => {
            const message = (0, test_helpers_1.createTestMessage)('test message '.repeat(50));
            await resourceManager.addMessage(TEST_CONTEXT_ID, message);
        ...
        it('should handle multiple messages efficiently', async () => {
            const messages = Array(5).fill(null).map((_, i) => (0, test_helpers_1.createTestMessage)(`test message ${i} `.repeat(10)));
    ...
    describe('Data Integrity', () => {
        it('should preserve message content after optimization', async () => {
            const message = (0, test_helpers_1.createTestMessage)('important test message');
            await resourceManager.addMessage(TEST_CONTEXT_ID, message);
            await resourceManager.optimizeResources();
            const context = await resourceManager.getContext(TEST_CONTEXT_ID);
        ...
        it('should maintain message order during optimization', async () => {
            const messages = Array(3).fill(null).map((_, i) => (0, test_helpers_1.createTestMessage)(`message ${i}`));
            for (const message of messages) {
                await resourceManager.addMessage(TEST_CONTEXT_ID, message);
            }
            await resourceManager.optimizeResources();
            const context = await resourceManager.getContext(TEST_CONTEXT_ID);
            expect(context?.messages.map(m => m.content)).toEqual(messages.map(m => m.content));
        });
    ...
    describe('Performance Impact', () => {
        it('should optimize resources within acceptable time', async () => {
            const message = (0, test_helpers_1.createTestMessage)('test message '.repeat(100));
            await resourceManager.addMessage(TEST_CONTEXT_ID, message);
        ...
        it('should handle concurrent message additions', async () => {
            const messages = Array(10).fill(null).map((_, i) => (0, test_helpers_1.createTestMessage)(`concurrent message ${i}`));
            await Promise.all(messages.map(msg => resourceManager.addMessage(TEST_CONTEXT_ID, msg)));
            const context = await resourceManager.getContext(TEST_CONTEXT_ID);
            expect(context?.messages).toHaveLength(messages.length);
        });
    ...
    describe('Context Recovery', () => {
        it('should handle context retrieval after optimization', async () => {
            const message = (0, test_helpers_1.createTestMessage)('test message for recovery');
            await resourceManager.addMessage(TEST_CONTEXT_ID, message);
            await resourceManager.optimizeResources();
            const context = await resourceManager.getContext(TEST_CONTEXT_ID);
        ...
        it('should maintain context integrity under memory pressure', async () => {
            const messages = Array(5).fill(null).map((_, i) => (0, test_helpers_1.createTestMessage)(`context message ${i}`));
            for (const message of messages) {
                await resourceManager.addMessage(TEST_CONTEXT_ID, message);
            }
            await resourceManager.optimizeResources();
        ...
        it('should recover context after multiple optimization cycles', async () => {
            const messages = Array(3).fill(null).map((_, i) => (0, test_helpers_1.createTestMessage)(`cycle message ${i}`));
            for (const message of messages) {
                await resourceManager.addMessage(TEST_CONTEXT_ID, message);
                await resourceManager.optimizeResources();
            }
...
//# sourceMappingURL=memory-compression.test.js.map