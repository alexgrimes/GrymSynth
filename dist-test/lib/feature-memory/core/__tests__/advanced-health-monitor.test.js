...
const advanced_health_monitor_1 = require("../advanced-health-monitor");
...
describe('AdvancedHealthMonitor', () => {
    ...
    beforeEach(() => {
        ...
        monitor = new advanced_health_monitor_1.AdvancedHealthMonitor(metricsCollector);
        // Mock the getMetricsHistory method
        jest.spyOn(monitor, 'getMetricsHistory').mockReturnValue(mockMetrics);
    });
    describe('Pattern Detection', () => {
        it('should detect cyclic patterns in metrics', async () => {
            const healthStatus = monitor.checkHealth();
            const enhancedMetrics = healthStatus.metrics;
        ...
        it('should detect upward trends in resource usage', async () => {
            ...
            jest.spyOn(monitor, 'getMetricsHistory').mockReturnValue(trendMetrics);
            const healthStatus = monitor.checkHealth();
            const predictions = healthStatus.metrics.prediction;
        ...
        it('should detect performance spikes', async () => {
            ...
            jest.spyOn(monitor, 'getMetricsHistory').mockReturnValue(spikeMetrics);
            const healthStatus = monitor.checkHealth();
            const predictions = healthStatus.metrics.prediction;
    ...
    describe('Resource Prediction', () => {
        it('should forecast resource needs', async () => {
            const healthStatus = monitor.checkHealth();
            const predictions = healthStatus.metrics.prediction.resource;
    ...
    describe('Recommendation Generation', () => {
        ...
        it('should include performance-based recommendations', async () => {
            ...
            jest.spyOn(monitor, 'getMetricsHistory').mockReturnValue(degradedMetrics);
            const healthStatus = monitor.checkHealth();
            const hasPerformanceRecs = healthStatus.recommendations.some(rec => rec.toLowerCase().includes('performance') || rec.toLowerCase().includes('latency'));
...
//# sourceMappingURL=advanced-health-monitor.test.js.map