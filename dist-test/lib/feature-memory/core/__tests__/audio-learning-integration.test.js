...
describe('Audio Learning Integration', () => {
    ...
    describe('Pattern Learning Lifecycle', () => {
        it('should accumulate and recognize patterns over multiple processing runs', async () => {
            ...
            const createTestData = (frequency) => ({
                id: `test-${frequency}`,
            ...
            expect(result3.learningMetrics?.knownPatternsCount || 0).toBeGreaterThan(result1.learningMetrics?.knownPatternsCount || 0);
        });
        it('should maintain learning state between sessions', async () => {
            ...
            const newManager = new audio_processing_manager_1.AudioProcessingManager(projectManager, healthMonitor, featureMemory);
        ...
        it('should handle concurrent audio processing while maintaining learning state', async () => {
            ...
            const results = await Promise.all(audioFiles.map(file => audioManager.processAudio(file)));
...
//# sourceMappingURL=audio-learning-integration.test.js.map