...
class PatternValidator {
     ...
     */
    removeRule(ruleSet, ruleCode) {
        const existingRules = this.rules.get(ruleSet);
        if (existingRules) {
            this.rules.set(ruleSet, existingRules.filter(rule => rule.code !== ruleCode));
        }
    }
    /**
    ...
    initializeDefaultRules() {
        ...
        this.addRule('pattern', {
            validate: (pattern) => pattern && typeof pattern === 'object',
        ...
        this.addRule('features', {
            validate: (features) => features instanceof Map && features.size > 0,
        ...
        this.addRule('confidence', {
            validate: (confidence) => typeof confidence === 'number' && confidence >= 0 && confidence <= 1,
