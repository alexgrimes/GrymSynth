...
exports.HealthStateManager = void 0;
class HealthStateManager {
    ...
    transition(to) {
        if (!this.canTransition(this._currentState, to)) {
            throw new Error(`Invalid state transition from ${this._currentState.status} to ${to.status}`);
        }
        const transition = {
    ...
    initializeDefaultGuards() {
        ...
        this.addGuardCondition({
            evaluate: () => {
                ...
                const recentTransitions = this.stateHistory.transitions.filter(t => Date.now() - t.timestamp <= window);
...
exports.HealthStateManager = HealthStateManager;
class StateHistoryImpl {
    constructor() {
        this._transitions = [];
        this._samples = [];
