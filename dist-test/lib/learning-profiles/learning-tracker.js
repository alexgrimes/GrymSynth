...
exports.ModelLearningTracker = void 0;
class ModelLearningTracker {
    constructor(storage) {
        this.storage = storage;
    }
    calculateConfidence(currentConfidence, success, complexity = 0.5) {
    ...
    calculateMastery(exposures, confidence) {
        if (exposures < 5 || confidence < 0.3)
            return 'novice';
        if (exposures < 20 || confidence < 0.7)
            return 'competent';
        return 'expert';
    }
    async findNewConnections(profile, interaction) {
        ...
        for (const relatedTopic of relatedTopics) {
            if (relatedTopic === currentDomain)
                continue;
            const existingConnection = profile.learningState.crossDomainConnections.get(`${currentDomain}-${relatedTopic}`);
            const connectionStrength = this.calculateConnectionStrength(profile, currentDomain, relatedTopic, interaction.success);
            const connection = {
                from: currentDomain,
                to: relatedTopic,
            ...
            profile.learningState.crossDomainConnections.set(`${currentDomain}-${relatedTopic}`, connection);
            newConnections.push(connection);
        }
        return newConnections;
    }
    calculateConnectionStrength(profile, domain1, domain2, success) {
        ...
        if (!domain1Knowledge || !domain2Knowledge)
            return 0.1;
        const baseStrength = (domain1Knowledge.confidence +
            domain2Knowledge.confidence) / 2;
        return success ? baseStrength : baseStrength * 0.8;
    }
    async updateModelLearning(modelId, interaction) {
        ...
        domain.confidence = this.calculateConfidence(domain.confidence, interaction.success, interaction.metadata?.complexity);
    ...
    async getModelAnalysis(modelId, domain) {
    ...
    async visualizeProfile(modelId) {
        const profile = await this.storage.loadProfile(modelId);
        if (!profile)
            return null;
        const domains = Array.from(profile.learningState.domains.entries()).map(([name, knowledge]) => ({
            name,
...
exports.ModelLearningTracker = ModelLearningTracker;
